...
### Fehlerbehebung
- Wenn die Seite mit HTTP 500 reagiert, fehlt meist der PDO-MySQL-Treiber oder die DB ist nicht erreichbar. Die Ausgabe zeigt eine Klartext-Ursache, zusätzlich landet alles im Log `logs/php-error.log`.
- Aktiviere bei Bedarf den Treiber via `php-mysql`/`pdo_mysql` und prüfe die Zugangsdaten in `config.php`.
